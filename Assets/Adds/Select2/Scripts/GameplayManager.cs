...
public class GameplayManager : SingletonNetwork<GameplayManager>
{
    ...
    [SerializeField]
    private Transform[] m_characterStartingPositions;

    private int m_numberOfPlayerConnected;
    private List<ulong> m_connectedClients = new List<ulong>();
    private List<PlayerController> m_playerControl = new List<PlayerController>();
    ...
    {
        foreach (var player in m_playerControl)
        {
            if (player != null)
            {
    ...
    [ClientRpc]
    private void SetPlayerUIClientRpc(int charIndex, string playercharacterName)
    {
        // Not optimal, but this is only called one time per character
        // We do this because we can not pass a GameObject in an RPC
        GameObject playercharacter = GameObject.Find(playercharacterName);

        PlayerHealth playerHealth =
            playercharacter.GetComponent<PlayerHealth>();

        m_playersUI[m_charactersData[charIndex].playerId].SetUI(
            m_charactersData[charIndex].playerId,
            m_charactersData[charIndex].iconAvatar,
            //m_charactersData[charIndex].iconDeathSprite,
            playerHealth.healthNet.Value,
            m_charactersData[charIndex].darkColor);

        // Pass the UI to the player
        playerHealth.playerUI = m_playersUI[m_charactersData[charIndex].playerId];
    }
    ...
    // Because of that, if we create the character when a player connects we could have a sync error
    // with the other clients because maybe the scene on the client is no yet loaded.
    // To fix this problem we wait until all clients call this method then we create the characters
                ...
                if (data.isSelected && data.clientId == client)
                {
                    GameObject playerChara =
                        NetworkObjectSpawner.SpawnNewNetworkObjectAsPlayerObject(
                            ...
                            m_characterStartingPositions[m_numberOfPlayerConnected].position,
                            data.clientId,
                            true);

                    PlayerController playerController =
                         playerChara.GetComponent<PlayerController>();
                     playerController.characterData = data;
                     playerController.gameplayManager = this;

                     m_playerControl.Add(playerController);
                     SetPlayerUIClientRpc(index, playerChara.name);
